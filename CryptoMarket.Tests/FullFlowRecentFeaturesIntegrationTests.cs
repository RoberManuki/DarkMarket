...
public class FullFlowRecentFeaturesIntegrationTests : IClassFixture<IntegrationTestWebAppFactory>
{
    ...
    {
        using var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("Cookie", "cryptomarket.uiLanguage=fr-FR");
    ...
    public async Task SetLanguage_UnsafeReturnUrl_RedirectsHome_AndStillSetsLanguageCookie()
    {
        ...
        Assert.Contains(response.Headers, h =>
            ...
            && h.Value.Any(v => v.Contains("cryptomarket.uiLanguage=en-US", StringComparison.Ordinal)));
    }
    ...
    public async Task Breadcrumb_DoesNotIncludeUiLangQueryText_OnAdminLanguagesRoute()
    {
        ...
        Assert.DoesNotContain("?uiLang=", html, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("uiLang%3D", html, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task CookieConsent_AssetsAndMarkup_ArePublishedWithExpectedContracts()
    {
        ...
        Assert.Contains("cryptoMarketIsDevelopment", hostHtml, StringComparison.Ordinal);
        ...
        Assert.Contains("window.cryptoMarketCookieConsent", script, StringComparison.Ordinal);
        ...
        Assert.Contains("cryptomarket.cookieConsent.v1", script, StringComparison.Ordinal);
    }
    ...
    private async Task<string> SetLanguageAndExtractCookieAsync(string languageCode, string returnUrl)
    {
        ...
        var setCookie = response.Headers
            ...
            .First(v => v.Contains("cryptomarket.uiLanguage=", StringComparison.Ordinal));
